...
class AlphaAlgoDesktopApp extends StatefulWidget {
  const AlphaAlgoDesktopApp({super.key});

  @override
  State<AlphaAlgoDesktopApp> createState() => _AlphaAlgoDesktopAppState();
}

class _AlphaAlgoDesktopAppState extends State<AlphaAlgoDesktopApp> {
  ...
  void _syncWs() {
    if (_authController.status == AuthStatus.authenticated) {
      ...
      if (token != null) _wsController.start(token);
    } else {
      _wsController.stop();
    }
  }
